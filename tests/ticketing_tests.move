...
module ticketing::test_laplace_low_participations {
    ...
    #[test]
    #[expected_failure(abort_code = ticketing::ticket::ERR_INSUFFICIENT_BALANCE)]
    public fun test2() {
        let mut scenario_test = init_test_helper();
        let scenario = &mut scenario_test;
        ...
        // process payment with lp points
        next_tx(scenario, TEST_ADDRESS1);
        {
            let mut customer = ts::take_from_sender<Customer>(scenario);
            let mut ticket_order = ts::take_from_sender<TicketOrder>(scenario);
            ...
            ticket::process_payment_with_loyalty_points(&mut customer, &mut ticket_order, recipient, ts::ctx(scenario));
